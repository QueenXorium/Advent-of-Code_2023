...
$contains_part_number = array_2d(NUMBER_OF_ROWS, NUMBER_OF_COLUMNS, FALSE); // boolean[][]

forEach ($number_metadata as $metadatum) {
    if (is_part_number($metadatum)) {
        $row = $metadatum["row"];
        $part_number = $metadatum["value"];
        $left_col = $metadatum["col_start"];
        $right_col = $metadatum["col_end"];
        for ($col = $left_col; $col <= $right_col; $col++) {
            $contains_part_number[$row][$col] = $metadatum;
        }
    }
}
...
for ($row = 0; $row < NUMBER_OF_ROWS; $row++) {
    for ($col = 0; $col < NUMBER_OF_COLUMNS; $col++) {
        ...
        // Using IDs to avoid storing the same distinct number more than once. Cant use value because "123*123" for example
        $surrounding_part_number_metadata_id_to_value = [];
        $coords = get_border_coordinates($row, $col, $row, $col, NUMBER_OF_ROWS, NUMBER_OF_COLUMNS);
        forEach ($coords as $coord) {
            $part_number_metadata = $contains_part_number[$coord[0]][$coord[1]];
            if ($part_number_metadata !== false) {
                $surrounding_part_number_metadata_id_to_value[$part_number_metadata["id"]] = $part_number_metadata["value"];
            }
        }
        // A gear is any * symbol that is adjacent to exactly two part numbers.
        $surrounding_part_numbers = array_values($surrounding_part_number_metadata_id_to_value);
        if (count($surrounding_part_numbers) == 2) {
            $pn1 = $surrounding_part_numbers[0];
            $pn2 = $surrounding_part_numbers[1];
            $gear_ratio = $pn1 * $pn2;
            $sum += $gear_ratio;
        }
    }
